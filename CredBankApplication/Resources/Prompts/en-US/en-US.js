...
function phonePrompts(value)
{
    ...
    if (phoneNumber.length == 10) {
        promptsArray = promptsArray.concat(
                ...
                alphanumericPrompts(phoneNumber.substring(3, 6)),
                promptBaseUrl + "miscellaneous/350ms" + audioFileExtension,
                alphanumericPrompts(phoneNumber.substring(6)));
...
/*    while (number > 0) {
    	...
    	while (number > 0) {
        //System.out.println("number at the begining : "+number);
        if(number>1000) {
        endDigits = number % 1000;
        }else if(number<1000 && number>99) {
        endDigits = number % 1000;
        }else {
        endDigits = number % 100;
        }
        ...
        if(magnitude==0) {
             ...
             promptsArray = threeDigitsPrompts(endDigits/10).concat(magnitudePrompts(magnitude), promptsArray);
        }
        }
        if(number<=1000) {
        number = number - endDigits;
...
function magnitudePrompts(number)
{
    switch (number) {
        ...
        case 2:
            return new Array(promptBaseUrl + "cardinals/lakh" + audioFileExtension);
        case 3:
            return new Array(promptBaseUrl + "cardinals/crore" + audioFileExtension);
        case 4:
